...
export default function CollectionView({
  ...
  return (
    <div className="page-wrapper space-y-6">
          ...
          {breadcrumb.map((c, i) => (
            <li key={i} className="flex items-center gap-1.5">
              ...
              {c.href ? (
                ...
                <span className="text-[#4B4B4B]" aria-current="page">{c.name}</span>
              )}
            </li>
          ))}
        </ol>
      ...
      {/* Title */}
      <div className="w-fit">
        <h1 className="font-montserrat font-medium text-[22px] lg:text-[28px] leading-tight text-black">
          {title}
        </h1>
      ...
      {intro && intro.trim() && (
        <div className="max-w-4xl font-inter text-[14px] lg:text-base leading-6 text-[#4B4B4B] whitespace-pre-line">
          {intro}
        </div>
      )}
